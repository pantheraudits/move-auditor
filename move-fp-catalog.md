...
| 2 | "Unchecked arithmetic — overflow possible" | Move aborts on overflow (DoS, not silent corruption). DoS is only a finding if attacker profits from the abort | Prove the attacker profits from the abort or that the abort permanently bricks state |
...
| # | Pattern that looks vulnerable | Why it's safe | When it IS a real bug |
|---|------------------------------|---------------|----------------------|
| 1 | Arithmetic overflow in financial calculation | Move aborts on overflow — state is NOT corrupted. The transaction simply fails | When the abort permanently bricks state (abort-before-checkpoint pattern — see DEFI-85/86) or when attacker profits from the DoS |
